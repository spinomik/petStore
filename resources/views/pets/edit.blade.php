    ...
    <div class="container">
        ...
        <form method="POST" action="{{ route('pets.update') }}">
            ...
            <div class="mb-3">
                ...
                <input type="number" class="form-control" id="id" name="id" value="{{ $pet->getId() }}" required
                    readonly>
            </div>

            <div class="mb-3">
                <label for="name" class="form-label">Nazwa</label>
                <input type="text" class="form-control" id="name" name="name" value="{{ $pet->getName() }}"
                    required>
            </div>
            ...
            <div class="mb-3">
                ...
                <select name="category" id="category" class="form-select" required>
                    @foreach ($categories as $key => $value)
                        <option value="{{ $key }}" {{ $pet->getCategoryId() == $key ? 'selected' : '' }}>
                            {{ $value }}
                        </option>
                    @endforeach
                </select>
            ...
            <div class="mb-3">
                ...
                <select name="status" id="status" class="form-select" required>
                    @foreach ($statuses as $status)
                        <option value="{{ $status }}" {{ $pet->getStatus() == $status ? 'selected' : '' }}>
                            {{ ucfirst($status) }}
                        </option>
                    @endforeach
                </select>
